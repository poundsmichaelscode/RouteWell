...
export function errorHandler(
  ...
  let normalized = error instanceof ApiError
    ? error
    : new ApiError(500, "Internal server error");

  if (isMalformedJson(error)) {
    normalized = new ApiError(400, "Malformed JSON request body", "INVALID_JSON");
  }
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    ...
    if (error.code === "P2025") {
      normalized = new ApiError(404, "Record not found", "NOT_FOUND");
    }
    if (error.code === "P2003") {
      normalized = new ApiError(
        409,
